...
import { MessageToEditType } from "../../pages/ChatById";
...
export type chatInputType = {
  ...
  onInput: () => void;
  editMessage: () => void;
  setMessageToEdit: (data: MessageToEditType) => void;
  messageToEdit: MessageToEditType;
};
...
function ChatInput({
  ...
  return (
    <Box component={"form"} onSubmit={onSubmit} className={"input_wrapper"}>
      {showSelectedFile && fileUrl && (
        <Box
          className={"preview_image"}
          onClick={() => setShowSelectedFile(false)}
        >
          <img src={fileUrl} alt={"image"} />
        </Box>
      )}
      {file && (
        <Box className={"file_name"} onClick={() => setShowSelectedFile(true)}>
          <Typography
      ...
      <Box display={"flex"} borderRadius={10} className={"input_wrapper_flex"}>
        <TextField
          ...
          multiline={true}
          label={null}
          value={messageToEdit?.message_id ? messageToEdit.value : message}
          onChange={(e) => {
            if (e.target.value) {
              onInput();
            }
            if (messageToEdit?.message_id) {
              setMessageToEdit({ ...messageToEdit, value: e.target.value });
            } else {
          ...
          //         position="start">{messageToEdit?.message_id ? 'Edit:' : 'Message:'}</InputAdornment>,
          // }}
          variant="standard"
        />
        {!result && !messageToEdit && (
          <Button
            component="label"
            variant="outlined"
        ...
        >
          {message || file || result || messageToEdit ? (
            <SendTwoToneIcon />
          ) : (
            <KeyboardVoiceIcon />
          )}
